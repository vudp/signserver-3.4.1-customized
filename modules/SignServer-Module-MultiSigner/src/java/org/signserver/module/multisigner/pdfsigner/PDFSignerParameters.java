...
/*
 * import com.itextpdf.text.*; import com.itextpdf.text.pdf.*; import
 ...
 *
 * @author rayback_2
 * @version $Id: PDFSignerParameters.java 1949 2011-11-23 09:12:18Z netmackan $
 */
public class PDFSignerParameters {
    ...
    private String visible_sig_custom_image_text;
    private String visible_sig_force_valid;
    private String visible_validation_symbol;
    private String visible_sig_text_color;
    private String signature_signerinfo_prefix;
    private String signature_datetime_prefix;
    private String signature_signreason_prefix;
    private String signature_location_prefix;
    ...
    private String tsa_provider;
    private String textDirection = PDFSigner.TEXT_DIRECTION_LEFTTORIGHT;
    ...
    private boolean use_custom_image_text = false;
    private boolean use_force_valid = false;
    private boolean use_validation_symbol = false;
    private boolean use_timestamp = false;
    private boolean use_timestamp_authorization = false;
    //private boolean signerinfo_show_signerinfo_only = false;
    //private boolean signerinfo_show_datetime_only = false;
    private Image custom_image = null;
    private boolean showSignerInfo = true;
    private boolean showSignDateTime = true;
    private boolean showReason = true;
    private boolean showLocation = false;
    private String textStatusPosition = null;
    ...
    private void extractAndProcessConfigurationProperties()
        ...
        /*
         * // timestamp url if
         * (config.getProperties().getProperty(PDFSigner.TSA_URL) != null) {
         * tsa_url = config.getProperties().getProperty(PDFSigner.TSA_URL);
         * use_timestamp = true; LOG.debug("Using tsa url : " + tsa_url); }
         *
         * if (use_timestamp &&
         ...
         * PDFSigner.TSA_USERNAME); tsa_password =
         * config.getProperties().getProperty( PDFSigner.TSA_PASSWORD);
         * use_timestamp_authorization = true; }
         */
        if (config.getProperties().getProperty(PDFSigner.TSA_ENABLE) != null) {
            if (config.getProperties().getProperty(PDFSigner.TSA_ENABLE).compareTo("True") == 0) {
                use_timestamp = true;
            }
        }

        if (config.getProperties().getProperty(PDFSigner.SHOW_SIGNER_INFO) != null) {
            if (config.getProperties().getProperty(PDFSigner.SHOW_SIGNER_INFO).compareTo("True") == 0) {
                showSignerInfo = true;
            } else {
        ...
        if (config.getProperties().getProperty(PDFSigner.SHOW_DATETIME) != null) {
            if (config.getProperties().getProperty(PDFSigner.SHOW_DATETIME).compareTo("True") == 0) {
                showSignDateTime = true;
            } else {
                showSignDateTime = false;
            }
        }

        if (config.getProperties().getProperty(PDFSigner.SHOW_REASON) != null) {
            if (config.getProperties().getProperty(PDFSigner.SHOW_REASON).compareTo("True") == 0) {
                showReason = true;
            } else {
                showReason = false;
            }
        ...
        if (config.getProperties().getProperty(PDFSigner.SHOW_LOCATION) != null) {
            if (config.getProperties().getProperty(PDFSigner.SHOW_LOCATION).compareTo("True") == 0) {
                showLocation = true;
            } else {
                showLocation = false;
            }
        ...
        if (config.getProperties().getProperty(PDFSigner.TEXT_STATUS_POSITION) != null) {
            textStatusPosition = config.getProperties().getProperty(PDFSigner.TEXT_STATUS_POSITION);
        }

        if (config.getProperties().getProperty(PDFSigner.TSA_PROVIDER) != null) {
            tsa_provider = config.getProperties().getProperty(PDFSigner.TSA_PROVIDER);
        }

        if (config.getProperties().getProperty(PDFSigner.TEXT_DIRECTION) != null) {
            textDirection = config.getProperties().getProperty(PDFSigner.TEXT_DIRECTION);
        }
        ...
        if (add_visible_signature) {
            ...
            if (config.getProperties().getProperty(
                    PDFSigner.IMAGE_AND_TEXT) != null) {
                visible_sig_custom_image_text = config.getProperties().getProperty(
                        PDFSigner.IMAGE_AND_TEXT);
            }

            if (config.getProperties().getProperty(
                    PDFSigner.TEXT_COLOR) != null) {
                visible_sig_text_color = config.getProperties().getProperty(
                        PDFSigner.TEXT_COLOR);
            } else {
                visible_sig_text_color = "BLACK";
            }

            if (visible_sig_custom_image_text != null) {
                if (visible_sig_custom_image_text.compareTo("True") == 0) {
                    use_custom_image_text = true;
                } else {
                    use_custom_image_text = false;
                }
            }

            if (config.getProperties().getProperty(
                    PDFSigner.VISIBLE_SIGNATURE_FORCE_VALID) != null) {
                visible_sig_force_valid = config.getProperties().getProperty(
                        PDFSigner.VISIBLE_SIGNATURE_FORCE_VALID);
            }

            if (visible_sig_force_valid != null) {
                if (visible_sig_force_valid.compareTo("True") == 0) {
                    use_force_valid = true;
                } else {
                    use_force_valid = false;
                }
            }

            if (config.getProperties().getProperty(
                    PDFSigner.VISUAL_STATUS) != null) {
                visible_validation_symbol = config.getProperties().getProperty(
                        PDFSigner.VISUAL_STATUS);
            }

            if (visible_validation_symbol != null) {
                if (visible_validation_symbol.compareTo("True") == 0) {
                    use_validation_symbol = true;
                } else {
                    use_validation_symbol = false;
                }
            }

            if (config.getProperties().getProperty(PDFSigner.SIGNER_INFO_PREFIX) != null) {
                signature_signerinfo_prefix = config.getProperties().getProperty(PDFSigner.SIGNER_INFO_PREFIX);
            } else {
                signature_signerinfo_prefix = null;
            }

            if (config.getProperties().getProperty(PDFSigner.DATETIME_PREFIX) != null) {
                signature_datetime_prefix = config.getProperties().getProperty(PDFSigner.DATETIME_PREFIX);
            } else {
                signature_datetime_prefix = null;
            }

            if (config.getProperties().getProperty(PDFSigner.SIGNREASON_PREFIX) != null) {
                signature_signreason_prefix = config.getProperties().getProperty(PDFSigner.SIGNREASON_PREFIX);
            } else {
                signature_signreason_prefix = null;
            }

            if (config.getProperties().getProperty(PDFSigner.LOCATION_PREFIX) != null) {
                signature_location_prefix = config.getProperties().getProperty(PDFSigner.LOCATION_PREFIX);
            } else {
                signature_location_prefix = null;
            }

            /*
             * if(config.getProperties().getProperty(PDFSigner.SIGNERINFO_SHOW_SIGNERINFO_ONLY)
             * != null) { signerinfo_show_signerinfo_only =
             * Boolean.parseBoolean(config.getProperties().getProperty(PDFSigner.SIGNERINFO_SHOW_SIGNERINFO_ONLY));
             * } else { signerinfo_show_signerinfo_only = false; }
             *
             * if(config.getProperties().getProperty(PDFSigner.SIGNERINFO_SHOW_DATETIME_ONLY)
             * != null) { signerinfo_show_datetime_only =
             * Boolean.parseBoolean(config.getProperties().getProperty(PDFSigner.SIGNERINFO_SHOW_DATETIME_ONLY));
             * } else { signerinfo_show_datetime_only = false; }
             */
    ...
    public boolean isUse_custom_image_text() {
        return use_custom_image_text;
    }

    public boolean isUse_force_valid() {
        return use_force_valid;
    }

    public boolean isUse_validation_symbol() {
        return use_validation_symbol;
    }

    public String getTextColor() {
        return visible_sig_text_color;
    }
    ...
    public String getSignerInfoPrefix() {
        return signature_signerinfo_prefix;
    }

    public String getDateTimePrefix() {
        return signature_datetime_prefix;
    }

    public String getSignReasonPrefix() {
        return signature_signreason_prefix;
    }

    public String getLocationPrefix() {
        return signature_location_prefix;
    }
    /*
     * public boolean get_signerinfo_show_signerinfo_only() { return
     * signerinfo_show_signerinfo_only; }
     *
     * public boolean get_signerinfo_show_datetime_only() { return
     * signerinfo_show_datetime_only; }
     */

    /**
     * @return if we should refuse PDF documents that contains multiple indirect
    ...
    public Image getCustom_image(String base64Image, int llx, int lly, int urx, int ury) {
        Image custom_image = null;
        try {
            byte[] imageByteArray;
            imageByteArray = Base64.decode(base64Image.getBytes());
            custom_image = Image.getInstance(imageByteArray);

            float newWidth = urx - llx;
            float newHeight = ury - lly;
            custom_image.scaleToFit(newWidth, newHeight);
    ...
    public boolean getShowSignerInfo() {
        return showSignerInfo;
    }

    public boolean getShowSignDateTime() {
        return showSignDateTime;
    }
    ...
    public String getTextStatusPosition() {
        return textStatusPosition;
    }
