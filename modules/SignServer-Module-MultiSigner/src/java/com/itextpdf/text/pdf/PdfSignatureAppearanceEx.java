...
/**
 * Class that takes care of the cryptographic options and appearances that form
 * a signature.
 */
public class PdfSignatureAppearanceEx {
     ...
     */
    PdfSignatureAppearanceEx(PdfStamperImp writer) {
        this.writer = writer;
     ...
     *
     * @param certificationLevel the values can be:
     * <code>NOT_CERTIFIED</code>,
    ...
    /**
     * The bytes of the file right before the signature is added (if raf is
     * null)
     */
    private byte[] bout;
    /**
     ...
     * The general sequence is: preClose(), getRangeStream() and close(). <p>
     *
    ...
    /**
     * Sets the certificate used to provide the text in the appearance. This
     * certificate doesn't take part in the actual signing process.
     *
    ...
    int[] pagesForInitials;

    public void setPagesForInitials(int[] pages) {
        pagesForInitials = pages;
    }
    ...
    /**
     * Gets a new signature field name that doesn't clash with any existing
     * name.
     *
    ...
    /**
     * The coordinates of the rectangle for a visible signature, or a
     * zero-width, zero-height rectangle for an invisible signature.
     */
    ...
    /**
     * rectangle that represent the position and dimension of the signature in
     * the page.
     */
     ...
     * @param fieldName the field name or
     * <CODE>null</CODE> to generate automatically a new field name
     */
     ...
     * <CODE>null</CODE> the mode is defaulted to
     * <CODE>RenderingMode.DESCRIPTION</CODE>
     */
     ...
     * to the /FRM resource. <p> Consult <A
     * HREF="http://partners.adobe.com/asn/developer/pdfs/tn/PPKAppearances.pdf">PPKAppearances.pdf</A>
     * for further details.
     *
    ...
    /**
     * Gets the main appearance layer. <p> Consult <A
     * HREF="http://partners.adobe.com/asn/developer/pdfs/tn/PPKAppearances.pdf">PPKAppearances.pdf</A>
     * for further details.
     *
     ...
     */
    private boolean enableLayer1 = false;
    private boolean enableLayer4 = false;
    private float[] checkMark = null;
    private float[] checkText = null;

    public void setEnableLayer1(boolean enable, float[] checkMark) {
        this.enableLayer1 = enable;
        this.checkMark = checkMark;
    }

    public void setEnableLayer4(boolean enable, float[] checkText) {
        this.enableLayer4 = enable;
    ...
    public PdfTemplate getAppearance() throws DocumentException {
        ...
        if (enableLayer4) {
            ...
            if (checkText == null) {
                ...
                //t.setBoundingBox(new Rectangle(0, 0, rect.getRight(), rect.getHeight()-(rect.getHeight() * (1 - TOP_SECTION))));
                t.setBoundingBox(new Rectangle(
                        (rect.getWidth() - 115) / 2,
                        rect.getHeight() - 35,
                        rect.getWidth() - ((rect.getWidth() - 115) / 2),
                        rect.getHeight()));
            } else {
                t.setBoundingBox(new Rectangle(checkText[0], checkText[1], checkText[2], checkText[3]));
            }
        ...
        if (frm == null) {
            ...
            float x = 0;
            float y = 0;
            if (enableLayer1 || enableLayer4) {
                if (checkMark == null) {
                    scale = Math.min(rect.getWidth(), rect.getHeight()) * 0.9f;
                    x = (rect.getWidth() - scale) / 2;
                    y = (rect.getHeight() - scale) / 2;
                    scale /= 100;
                } else {
                    scale = checkMark[0];
                    x = checkMark[1];
    ...
    /**
     * Gets the
     * <CODE>PdfStamper</CODE> associated with this instance.
     *
     * @return the
     * <CODE>PdfStamper</CODE> associated with this instance
     */
     ...
     * <CODE>null</CODE> is the document is created in memory
     */
    ...
    /**
     * Setter for the field lock dictionary. <p><strong>Be aware:</strong> if a
     * signature is created on an existing signature field, then its /Lock
     * dictionary takes the precedence (if it exists).</p>
     *
     ...
     *
     * @return
     * <CODE>true</CODE> if the document is in the process of closing,
     ...
     * general sequence is: preClose(), getDocumentBytes() and close(). <p> If
     * calling preClose() <B>dont't</B> call PdfStamper.close(). <p>
     * <CODE>exclusionSizes</CODE> must contain at least the
     * <CODE>PdfName.CONTENTS</CODE> key with the size that it will take in the
     * document. Note that due to the hex string coding this size should be
     * byte_size*2+2.
     *
     * @param exclusionSizes a
     * <CODE>HashMap</CODE> with names and sizes to be excluded in the signature
    ...
    public void preClose(HashMap<PdfName, Integer> exclusionSizes) throws IOException, DocumentException {
        ...
        if (fieldExists) {
            ...
            if (pagesForInitials == null) {
                ...
                for (int i = 0; i < pagesForInitials.length; i++) {
                    writer.addAnnotation(sigField, pagesForInitials[i]);
                }
            }
        }
     ...
     * and the permissions. This method is only used for Certifying signatures.
     *
     ...
     * general sequence is: preClose(), getDocumentBytes() and close(). <p>
     * <CODE>update</CODE> is a
     * <CODE>PdfDictionary</CODE> that must have exactly the same keys as the
     * ones provided in {@link #preClose(HashMap)}.
     *
     * @param update a
     * <CODE>PdfDictionary</CODE> with the key/value that will fill the holes
