...
public class PDFSigner extends BaseSigner {
    ...
    public static final String REASONDEFAULT = "Signed by TRUSTEDHUB";
    ...
    public static final String VISIBLE_SIGNATURE_CUSTOM_IMAGE_TEXT = "VISIBLE_SIGNATURE_CUSTOM_IMAGE_TEXT";
    public static final String VISIBLE_SIGNATURE_TEXT_COLOR = "VISIBLE_SIGNATURE_TEXT_COLOR";
    public static final String VISIBLE_SIGNATURE_FORCE_VALID = "VISIBLE_SIGNATURE_FORCE_VALID";
    public static final String VISIBLE_VALIDATION_SYMBOL = "VISIBLE_VALIDATION_SYMBOL";
    ...
    public ProcessResponse processData(ProcessRequest signRequest,
        ...
        // check license for PDFSigner
        LOG.info("Checking license for PDFSigner.");
        ...
        try {
            ...
            if(signedbytes == null)
            	return new GenericSignResponse(sReq.getRequestID(), archiveId, ResponseCode, ResponseMessage);
        	...
        	return new GenericSignResponse(sReq.getRequestID(), archiveId, Defines.CODE_PDFCANNOTSIGN, Defines.ERROR_PDFCANNOTSIGN);
            //throw new IllegalRequestException("Could not sign document: "+ e.getMessage(), e);
        } catch (BadPasswordException ex) {
        	return new GenericSignResponse(sReq.getRequestID(), archiveId, Defines.CODE_PDFPASS, Defines.ERROR_PDFPASS);
            //throw new IllegalRequestException("A valid password is required to sign the document: " + ex.getMessage(), ex);
        } catch (UnsupportedEncodingException ex) {
        	return new GenericSignResponse(sReq.getRequestID(), archiveId, Defines.CODE_PDFPASSENCODING, Defines.ERROR_PDFPASSENCODING);
            //throw new IllegalRequestException("The supplied password could not be read: " + ex.getMessage(), ex);
        } catch (IOException e) {
        	return new GenericSignResponse(sReq.getRequestID(), archiveId, Defines.CODE_PDFSIGN, Defines.ERROR_PDFSIGN);
            //throw new IllegalRequestException("Could not sign document: " + e.getMessage(), e);
        }
    ...
    protected byte[] addSignatureToPDFDocument(PDFSignerParameters params,
        ...
        X509Certificate x509 = (X509Certificate) this.getSigningCertificate();
        if (certs == null) {
        	ResponseCode = Defines.CODE_PDFCERTNULL;
        	ResponseMessage = Defines.ERROR_PDFCERTNULL;
        	return null;
        ...
        if (params.isAdd_visible_signature()) {
            ...
            if (params.isUse_custom_image()) {
            	if(params.isUse_custom_image_text()) {
            		if(!params.isUse_validation_symbol()) {
                    ...
                    FontFactory.register(EXTERN_FONT);
                    Font textFont = FontFactory.getFont(EXTEN_FONT_NAME, BaseFont.IDENTITY_H, 
                        BaseFont.EMBEDDED, 10);
                    textFont.setColor(getColor(params.getTextColor()));
                    sap.setLayer2Font(textFont);
                    String content = "Ký bởi: "+getSubjectName(x509.getSubjectDN().getName())+"\n"+"Ký ngày: "+formatter.format(cal.getTime());
                    if(params.isUse_force_valid()) {
