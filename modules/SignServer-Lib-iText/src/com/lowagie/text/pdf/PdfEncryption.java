...
import com.lowagie.text.pdf.crypto.AESCipherCBCnoPad;
...
public class PdfEncryption {
    ...
    public boolean readKey(PdfDictionary enc, byte[] password) throws BadPasswordException {
        try {
            if (password == null)
                password = new byte[0];
            byte[] oValue = com.lowagie.text.DocWriter.getISOBytes(enc.get(PdfName.O).toString());
            byte[] uValue = com.lowagie.text.DocWriter.getISOBytes(enc.get(PdfName.U).toString());
            byte[] oeValue = com.lowagie.text.DocWriter.getISOBytes(enc.get(PdfName.OE).toString());
            byte[] ueValue = com.lowagie.text.DocWriter.getISOBytes(enc.get(PdfName.UE).toString());
            byte[] perms = com.lowagie.text.DocWriter.getISOBytes(enc.get(PdfName.PERMS).toString());
