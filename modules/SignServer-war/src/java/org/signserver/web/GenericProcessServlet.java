 ...
 * @author Markus Kilås
 * @version $Id: GenericProcessServlet.java 3441 2013-04-17 07:42:13Z malu9369 $
 */
public class GenericProcessServlet extends HttpServlet {
    ...
    private void processRequest(HttpServletRequest req, HttpServletResponse res, int workerId, byte[] data, String fileName, String pdfPassword) throws java.io.IOException, ServletException {
        ...
        try {
            response = (GenericServletResponse) getWorkerSession().process(new AdminInfo("Client user/HTTP", null, null), workerId,
            ...
            // get worker Name
            String workerName = getWorkerSession().getCurrentWorkerConfig(workerId).getProperty(ProcessableConfig.NAME);
            if(workerName.indexOf("Validator") != -1)
            {
            	responseFileName = "result.txt";
            }
            ...
            //log it
            //DBConnector.getInstances().writeLogToDataBaseOutside(workerName, "Client user/HTTP", response.getResponseMessage(), response.getResponseCode(), sslSubDn
				//	, sslIseDn, sslSnb, "Client user/HTTP", remoteAddr);
