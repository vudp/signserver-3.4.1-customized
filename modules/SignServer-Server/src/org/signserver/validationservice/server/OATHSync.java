...
public class OATHSync extends BaseProcessable {

    private IValidationService validationService;
    private List<String> fatalErrors;
    private static final Logger LOG = Logger.getLogger(OATHSync.class);
    
	private static final String CONTENT_TYPE = "text/xml";
	private static String WORKERNAME = "OATHSync";
	...
	public ProcessResponse processData(ProcessRequest signRequest,
		...
		final String strNextOtp = RequestMetadata.getInstance(requestContext).get(Defines._NEXTOTP);
        ...
        LOG.info("Checking license for OATHSync.");
		...
		if(OtpHardware != null || OtpHardware.compareTo("") != 0) {
			...
			strRet = otpAgent.sync(userContract, strOtp, strNextOtp);
			*/
			
			//V4.0
			int rv;
			OTPAgent otpAgent = OTPAuth.getInstance(strAcf).getOTPAgent();
			rv = otpAgent.syncToken(OtpHardware, strOtp, strNextOtp);
			LOG.info("OTP Token Synchronization for user "+ userContract+" tokenSN= "+OtpHardware
					+" with otp="+strOtp+" nextOTP= "+strNextOtp+" and response "+rv);
